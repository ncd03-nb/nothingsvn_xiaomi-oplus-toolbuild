...
# Read a build flag from config.env. An uppercase environment variable of the
# same name wins, so a build can toggle the flag without editing the file.
config_flag() {
    local key="$1"
    local env_name="${key^^}"
    local value="${!env_name}"
    if [[ -z $value ]]; then
        value=$(grep "^${key}=" "${work_dir:-$WORK_DIR}/config.env" | cut -d '=' -f 2 | tr -d '[:space:]')
    fi
    echo "${value:-false}"
}
